...
while true; do
    ...
    echo "$containers" | while IFS='|' read -r name image; do
        ...
        # 2. Hacer pull silencioso de la imagen para actualizar la referencia local del tag
        # (Esto NO descarga datos pesados si la imagen no ha cambiado, solo comprueba los hashes)
        latest_image_id=$(docker pull "$image" --quiet 2>/dev/null)

        # 3. Comparar las IDs reales de las imágenes
